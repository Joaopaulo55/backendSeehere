...
// ✅ NOVAS ROTAS ADICIONADAS AQUI - ATIVAÇÃO DE USUÁRIOS
...
// Ativar todos os usuários (EMERGÊNCIA)
router.post('/activate-all-users', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Todos os usuários (${result.count}) foram ativados`,
      users: allUsers
    });
  } catch (error) {
...
// Rota de emergência HTTP - ATIVAÇÃO COMPLETA
router.post('/emergency-activate-all', async (req, res) => {
  try {
    console.log('🚨 ACIONADA CORREÇÃO DE EMERGÊNCIA VIA HTTP...');
    
    // 1. Ativar TODOS os usuários
    const activationResult = await prisma.user.updateMany({
      data: { isActive: true }
    });
    
    // 2. Garantir que emails admin sejam ADMIN
    const adminEmails = ['admin@seehere.com', 'xhanckin@gmail.com'];
    ...
    for (const email of adminEmails) {
      await prisma.user.upsert({
        ...
        create: {
          ...
          role: 'ADMIN',
          isActive: true,
          passwordHash: await bcrypt.hash('temp_password_123', 12)
        }
      });
    }
    ...
    res.json({
      success: true,
      message: `Sistema de emergência ativado. ${activationResult.count} usuários corrigidos.`,
      users: allUsers
    });
